...
import { GET_HEADER_LOGO_QUERY, GET_MEMBERSHIP_PLAN, GET_POSTS_CHANNELLIST_QUERY, GET_POSTS_LIST_QUERY } from '@/pages/api/query';
...
function MembershipPage() {
    ...
    console.log(tenantId, "jbvdfvbdf")
    ...
    useEffect(() => {
        const membershipApi = async () => {
            ...
            if (tenantId !== 0 && tenantId !== null && tenantId !== undefined) {
                ...
                console.log(res, "bcdfnjdchbfhc")
                if (res) {
                    setLoader(false)
                }
                setMemberData(res?.MembershipLevelList)
            }
        }
        membershipApi()
                        ...
                        <div className='grid grid-cols-3 gap-[24px] items-end max-md:grid-cols-1'>
                            {
                                loader ? (
                                    ...
                                    <>
                                        {
                                            memberData?.MemnershipDtails?.map((data, index) => (
                         ...
                         text-[#120B14]'>${data?.InitialPayment}</span>/month</h2>
